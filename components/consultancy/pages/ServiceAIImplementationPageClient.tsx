    ...
    <ConsultancyLoadedShell label="AI IMPLEMENTATION">
          ...
          <div style={{ display: "grid", gridTemplateColumns: layout === "desktop" ? "320px 1fr" : "1fr" }}>
            ...
            <div style={{ padding: "52px 36px" }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <Link href="/contact" style={{ background: L, color: "#000", textDecoration: "none", padding: "12px 18px", borderRadius: 999, fontFamily: MN, fontSize: 11, fontWeight: 700 }}>Start a project <Arr sz={10} cl="#000" /></Link>
      ...
      <section style={{ padding: `42px ${gv}px`, background: DK }}>
        <div style={{ display: "grid", gridTemplateColumns: gridCols(layout, 4, 2), gap: 1, background: "rgba(255,255,255,0.06)", borderRadius: 18, overflow: "hidden" }}>
          {METRICS.map((m) => <div key={m.l} style={{ background: DK, padding: 22 }}><div style={{ fontFamily: MN, fontSize: 42, color: L, fontWeight: 700 }}>{m.v}</div><div style={{ fontFamily: MN, fontSize: 11, color: "rgba(255,255,255,0.8)" }}>{m.l}</div><div style={{ fontFamily: SN, fontSize: 11, color: "rgba(255,255,255,0.5)" }}>{m.s}</div></div>)}
        </div>
      </section>
      ...
      <section style={{ padding: `${pv}px ${gv}px`, background: BG2, borderRadius: "24px 24px 0 0", marginTop: -24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: gridCols(layout, 3, 2), gap: 1, background: PL, borderRadius: 18, overflow: "hidden", border: `1px solid ${PL}` }}>
          {DELIVER.map(([t, d], i) => <div key={t} style={{ background: BG, padding: 22 }}><div style={{ fontFamily: MN, fontSize: 10, color: L2 }}>{String(i + 1).padStart(2, "0")} -</div><div style={{ fontFamily: MN, fontSize: 15, margin: "6px 0" }}>{t}</div><div style={{ fontFamily: SN, fontSize: 12.5, color: "rgba(0,0,0,0.57)" }}>{d}</div></div>)}
        </div>
      </section>
      ...
      <section style={{ padding: `${pv}px ${gv}px`, background: DK, borderRadius: "24px 24px 0 0", marginTop: -24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: gridCols(layout, 3, 2), gap: 1, background: "rgba(255,255,255,0.06)", borderRadius: 18, overflow: "hidden" }}>
          {PERSONAS.map(([t, d]) => <div key={t} style={{ background: DK, padding: 22 }}><div style={{ fontFamily: MN, color: "#fff", marginBottom: 6 }}>{t}</div><div style={{ fontFamily: SN, color: "rgba(255,255,255,0.62)", fontSize: 13 }}>{d}</div></div>)}
        </div>
      </section>
